  ...
  subroutine update_hlm_dynamics(nsites,sites,fcolumn,bc_out)
    ...
    do s = 1,nsites
       ...
       do while(associated(currentPatch))
          ...
          write(fates_log(),*) 'update_hlm_dynamics: pre-bc_out(s)%esai_pa(ifp)',bc_out(s)%esai_pa(ifp)
          ...
          write(fates_log(),*) 'update_hlm_dynamics: post-bc_out(s)%esai_pa(ifp)',bc_out(s)%esai_pa(ifp)
